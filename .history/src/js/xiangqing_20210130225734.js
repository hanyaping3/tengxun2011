...
bigimg.mousemove(function (event) {
    point = {
      ...
      if (!(point.x < maskNode[0].offsetWidth / 2 || point.x >= bigimg[].offsetWidth - maskNode.offsetWidth / 2)) {
        //遮罩层的位置
        maskNode.style.left = point.x - maskNode.offsetWidth / 2 + "px"
        //放大镜的位置
        // glassimg.style.left = - glassimg.offsetWidth  * xishu.x + glassNode.offsetHeight / 2 + "px";

        glassimg.style.left = -parseFloat(maskNode.style.left) * n + "px"

    }
    if (!(point.y < maskNode.offsetHeight / 2 || point.y >= bigimg.offsetHeight - maskNode.offsetHeight / 2)) {
        // 遮罩层的位置
        maskNode.style.top = point.y - maskNode.offsetHeight / 2 + "px"
        //放大镜的位置
        // glassimg.style.top = - glassimg.offsetHeight * xishu.y + glassNode.offsetHeight / 2  + "px"
        glassimg.style.top = -parseFloat(maskNode.style.top) * n + "px"
    }
})
